...
const createButton = (text, className, iconClass = null) => {
...
const createDragHandle = (type = "row") => {
  const handle = document.createElement("div");
  handle.className = type === "row" ? "row-drag-handle" : "col-drag-handle";
  handle.title =
...
const initRowControlButtons = (rowWrapper, layoutContainer) => {
  ...
  const addAbove = createButton("Add Row Above", "btn", "fa fa-plus");
  ...
  const addBelow = createButton("Add Row Below", "btn", "fa fa-plus");
...
const saveBtn = createButton("Save Layout", "save-layout-btn");
...
function createRow() {
  ...
  let rowDragHandle = createDragHandle("row");
  ...
  function setColumns(count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const plusBtn = createButton("", "col-plus-btn");
      ...
      if (count > 1) {
        const colDragHandle = createDragHandle("col");
  ...
  const deleteBtn = createButton("Delete Row", "delete-btn", "fa fa-trash");
